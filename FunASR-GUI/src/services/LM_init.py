...
class ASRModelManager:
    def __init__(self):
        ...
        # 从配置读取线程设置
        omp_threads = self.settings.value('ASRThreadSettings/omp_threads', '12')
        mkl_threads = self.settings.value('ASRThreadSettings/mkl_threads', '12')
        torch_threads = self.settings.value('ASRThreadSettings/torch_threads', '12')
    ...
    def init_model_configs(self):
        # 从配置读取基础路径
        base_dir = self.settings.value('ASRModelPaths/base_dir', '/media/fl01/data01/WorkSpace/FunASR/model')
...
class TTSModelManager:
    def __init__(self):
        ...
        omp_threads = self.settings.value('TTSThreadSettings/omp_threads', '12')
        mkl_threads = self.settings.value('TTSThreadSettings/mkl_threads', '12')
        torch_threads = self.settings.value('TTSThreadSettings/torch_threads', '12')
    ...
    def init_model_configs(self):
        base_dir = self.settings.value('TTSModelPaths/base_dir', '/media/fl01/data01/WorkSpace/CosyVoice/pretrained_models')

        model_cosyvoice_300M = self.settings.value('TTSModelConfigs/model_cosyvoice_300M', 'CosyVoice-300M')
        model_cosyvoice_300M_25Hz = self.settings.value('TTSModelConfigs/model_cosyvoice_300M_25Hz', 'CosyVoice-300M-25Hz')
        model_cosyvoice_300M_instruct = self.settings.value('TTSModelConfigs/model_cosyvoice_300M_instruct', 'CosyVoice-300M-Instruct')
        model_cosyvoice_300M_SFT = self.settings.value('TTSModelConfigs/model_cosyvoice_300M_SFT', 'CosyVoice-300M-SFT')
